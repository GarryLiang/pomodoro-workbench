...
> **v1.3.1 新增/修复：计时按真实时间计算不再漂移、跨平台字体自适应、窗口位置记忆、异常日志。**
> v1.2.0：置顶迷你悬浮窗；v1.1.0：4 套界面主题与可自定义闹钟提醒。
...
3. 悬浮窗内可直接 **开始/暂停**、**跳过**、**回主界面**；点 **📌** 可切换是否置顶；
4. 按住悬浮窗空白处可拖动位置（自动记忆，支持多显示器副屏），点 **×** 或右键可隐藏。

### 数据存储位置
- 所有数据保存在用户主目录 `~/.pomodoro_workbench/data.db`（SQLite 单文件）。
- 备份 = 拷贝该文件；删除该文件即恢复出厂状态。
- 运行日志（用于排查异常）位于同目录 `app.log`，最多保留 3 个 256KB 文件。
- 窗口位置与尺寸、主题、悬浮窗位置等偏好也保存在同一数据库的 settings 表中。
...
│   ├── pomodoro.py          # 番茄钟状态机引擎（按单调时钟计时）
│   ├── logger.py            # 运行日志（异常写入本地文件）
